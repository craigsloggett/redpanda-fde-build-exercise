...
func challengeMessages(input Input, first Verdict) []Message {
	user := fmt.Sprintf(`%s
...
func describeEdit(input Input) string {
	...
	return fmt.Sprintf("Article: %s\nEditor: %s (%s)\nEdit summary: %s\nSize change: %+d bytes\n\n%s:\n%s",
		input.Title, input.User, editor, comment, input.BytesDelta, diffLabel, input.Diff)
}
